...
with open(path) as colours_file: #Loads the colour file in read only mode
    ...
    for line in colours_file:
        if line[0] == "$": #Checks that the line is a colour value
            ...
            colour = Colour.from_rgb(int(colour_data[0]), int(colour_data[1]), int(colour_data[2])) #Creates the colour as a discord object from the data, ignores the alpha value as it is not supported
